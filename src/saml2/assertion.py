...
class Policy:
    ...
    def _subject_id_or_pairwise_id(
        ava: AttributeValues,
        required: Optional[List[AttributeAsDict]],
    ) -> tuple[AttributeValues, Optional[List[AttributeAsDict]]]:
        """
        ...
        if subject_id and pairwise_id:
            if all(
                [
                    friendly_name in ava
                    for friendly_name in [
                        subject_id["friendly_name"],
                        pairwise_id["friendly_name"],
                    ]
                ]
            ):
    ...
    def filter(
        ...
        if (
            self.metadata_store
            and self.metadata_store.subject_id_requirement_type(sp_entity_id) == "any"
        ):
            subject_ava, required = self._subject_id_or_pairwise_id(
                subject_ava, required
            )
